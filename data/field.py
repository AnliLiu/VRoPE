...
class RawField(object):
    ...
    def __init__(self, preprocessing=None, postprocessing=None):
        self.preprocessing = preprocessing  # 应用于examples的pipeline，在创建example之前使用
        self.postprocessing = postprocessing  # 分配给batch之前使用，是应用于a list of examples的pipeline
...
class ImageDetectionsField(RawField):
    def __init__(self, preprocessing=None, postprocessing=None, detections_path=None, max_detections=100,
                 ...
                 global_detections_path='/home/mlw/project/meshed-memory-transformer-master/feature/f30ktalk_att',
                 local_detections_path='/home/mlw/project/meshed-memory-transformer-master/feature/f30kbu_att',
                 rois_path='/media/liuli/pydata/pyproject/data/syden50_pos'):
        self.max_detections = max_detections
        self.detections_path = detections_path
        self.global_detections_path = global_detections_path
        self.local_detections_path = local_detections_path
    ...
    # def preprocess(self, x, avoid_precomp=False):  # 从hdf5文件提取检测特征
    #     image_id = int(x.split('_')[-1].split('.')[0])  # 输入x是'coco/images/val2014/COCO_val2014_000000012464.jpg'
    #     try:
    #
    #         f = h5py.File(self.detections_path, 'r')
    #         precomp_data = f['%d_features' % image_id][()]  # 第一次调用的特征是22*2048，  第二次调用的特征是15*2048，说明特征个数是不固定的
    ...
    #     if delta > 0:  # 大于0，表示检测的个数不够最大个数50,填充0到50行
    ...
    def preprocess(self, x, avoid_precomp=False):  # 从hdf5文件提取检测特征
        ...
        #image_name = 'NWPU_1410.jpg'
        #
        #修改
        image_name='1294.tif'
        # image_name = '618.tif'
        # 局部特征处理
        fl = np.load(self.local_detections_path + '/%s.npy' % image_name)
        lo_precomp_data = fl

        # sydney，UCM全局特征
        fg = np.load(self.global_detections_path + '/%s.npz' % image_name)  # 245,512

        gl_precomp_data = fg['feat'][49:245, :]
        # gl_precomp_data = fg['feat']
        # gl_precomp_data = gl_precomp_data.reshape(196,2048)

        rois = np.load(self.rois_path + '/%s.npy' % image_name)
        # noise
        # gl_precomp_data = fg['feat'].reshape(fg['feat'].shape[0] * fg['feat'].shape[1], fg['feat'].shape[2])  # 196,2048


        ###################################   tuihua
        # image_id = int(x.split('/')[-1].split('.')[0])-1
        # image_name = x.split('/')[-1]
        # fl = np.load(self.local_detections_path + '/%s.npy' % image_name)
        # lo_precomp_data = fl
        #
        # fg = np.load(self.global_detections_path + '/%s.npy' % image_id)
        # gl_precomp_data = fg.reshape(fg.shape[0] * fg.shape[1], fg.shape[2])
        ###################################


        #  全局特征处理  sydney
        # fg = np.load(self.global_detections_path + '/%s.npz' % image_id)
        ...
        #
        #
        # # delta_lo = self.max_detections - lo_precomp_data.shape[0]  # 50-22=28
        # # if delta_lo > 0:  # 大于0，表示检测的个数不够最大个数50,填充0到50行
        # #     lo_precomp_data = np.concatenate([lo_precomp_data, np.zeros((delta_lo, lo_precomp_data.shape[1]))], axis=0)
        # # else:
        # #     lo_precomp_data=lo_precomp_data[:self.max_detections,]
        #
        #
        # #  全局特征处理  sydney
        # # fg = np.load(self.global_detections_path + '/%s.npz' % image_id)
        ...
        # # gl_precomp_data = fg['feat'][:49, :]
        # gl_precomp_data=fg['feat'][49:245,:]
        # # gl_precomp_data=gl_precomp_data[: 32, ]
        #
        # # # RSICD
        # # image_name = x.split('/')[-1]
        # # # local
        ...
        return lo_precomp_data.astype(np.float32), gl_precomp_data.astype(np.float32),rois.astype(np.float32)
...
class TextField(RawField):
    ...
    def process(self, batch, device=None):
        padded = self.pad(batch)  # text batch，增加了bos， eos， pad 的text， 10 13
        tensor = self.numericalize(padded, device=device)  # text中的单词变为词汇表的索引， 变为10 13 张量
        return tensor
        ...
        for x in minibatch:
            if self.pad_first:
                ...
                padded.append(
                    ...
                    list(x[-max_len:] if self.truncate_first else x[:max_len]) +
                    ([] if self.eos_token is None else [self.eos_token]) +
                    [self.pad_token] * max(0, max_len - len(x)))  # 句子开头是bos，结尾是eos， 不足最大长度13的，填充pad
    ...
    def numericalize(self, arr, device=None):
        ...
        if self.use_vocab:
            arr = [[self.vocab.stoi[x] for x in ex] for ex in arr]  # 将 text batch 的word转换为词汇表中的数字索引，保存在arr
