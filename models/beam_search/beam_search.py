...
class BeamSearch(object):
    ...
    def apply(self, visual: utils.TensorOrSequence, visual_gl: utils.TensorOrSequence, ROIS, out_size=1, return_probs=False, **kwargs):
        ...
        with self.model.statefulness(self.b_s):
            for t in range(self.max_len):
                visual,visual_gl, outputs = self.iter(t, visual,visual_gl,ROIS, outputs, return_probs, **kwargs)
    ...
    def iter(self, t: int, visual: utils.TensorOrSequence,visual_gl:utils.TensorOrSequence, ROIS ,outputs, return_probs, **kwargs):
        cur_beam_size = 1 if t == 0 else self.beam_size

        word_logprob = self.model.step(t, self.selected_words, visual, visual_gl,ROIS,None, mode='feedback', **kwargs)
        ...
        visual_gl = self._expand_visual(visual_gl, cur_beam_size, selected_beam)
        ...
        return visual,visual_gl, outputs
