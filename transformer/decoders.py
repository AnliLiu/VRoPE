...
class MeshedDecoderLayer(Module):
    ...
    def forward(self, input, enc_output, mask_pad, mask_self_att, mask_enc_att):  # # decoder(seq, enc_output, mask_enc)
        self_att = self.self_att(input, input, input, mask_self_att, input_gl=None,
                                 isencoder=True)  # 跳到MultiHeadAttention
        ...
        enc_att = (enc_att1 * alpha1 + enc_att2 * alpha2 + enc_att3 * alpha3) / np.sqrt(3)  # 10 15 512
        # enc_att = (enc_att1 * alpha1) / np.sqrt(3)  # 10 15 512
...
class MeshedDecoder(Module):
    ...
    def forward(self, input, gl, encoder_output, mask_encoder):
        # input (b_s, seq_len) 10 30     #   10 30; 10 3 50 512; 10 1 1 50
        x = gl.permute(0, 2, 1)
        feats = self.pooling(x.unsqueeze(3))
        feats = feats.squeeze(dim=2)
        ...
        # print (trg_mask)
        capts_embed = self.word_emb(input)
        capts_embed = capts_embed + self.dropout(feats.expand_as(capts_embed))
        capts_embed = capts_embed + self.self_attn(capts_embed, capts_embed, capts_embed, mask_self_attention)
        capts_embed = capts_embed + self.pos_emb(seq)

        out = capts_embed
        #out = self.word_emb(input) + self.pos_emb(seq) + feats.expand_as(capts_embed)  # 10 30 512; 10 15 512     50,1,512
